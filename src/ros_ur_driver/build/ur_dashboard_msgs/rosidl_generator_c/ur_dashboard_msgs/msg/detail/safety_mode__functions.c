  ...
  ur_dashboard_msgs__msg__SafetyMode__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ur_dashboard_msgs__msg__SafetyMode * data =
      (ur_dashboard_msgs__msg__SafetyMode *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_dashboard_msgs__msg__SafetyMode__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          ur_dashboard_msgs__msg__SafetyMode__fini(&data[i]);
        }
        free(data);
        return false;
      }
