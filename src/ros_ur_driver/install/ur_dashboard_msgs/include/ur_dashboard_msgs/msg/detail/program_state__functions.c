  ...
  ur_dashboard_msgs__msg__ProgramState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ur_dashboard_msgs__msg__ProgramState * data =
      (ur_dashboard_msgs__msg__ProgramState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_dashboard_msgs__msg__ProgramState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          ur_dashboard_msgs__msg__ProgramState__fini(&data[i]);
        }
        free(data);
        return false;
      }
